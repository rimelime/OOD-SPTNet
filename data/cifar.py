...
from config import cifar_10_root, cifar_100_root, svhn_root
...
def get_cifar_10_datasets(train_transform, test_transform, ood_transform, train_classes=(0, 1, 8, 9),
    ...
    ood_training_set = CustomCIFAR10(root=cifar_10_root, ood_index=len(train_classes), 
                                     transform=ood_transform, train=True, download=True)
    val_dataset_labelled_split = subsample_dataset(ood_training_set, val_idxs)
    # val_dataset_labelled_split.ood_label = len(train_classes)
    # val_dataset_labelled_split.targets = [len(train_classes) for x in val_dataset_labelled_split.targets]
    ...
    all_datasets = {
        ...
        'ood_train': val_dataset_labelled,
        'test': test_dataset,
    }
...
def get_cifar_100_datasets(train_transform, test_transform, ood_transform, train_classes=range(80),
                       prop_train_labels=0.8, split_train_val=False, seed=0):
    ...
    # val_dataset_labelled_split.transform = ood_transform
    # val_dataset_labelled_split.ood_label = len(train_classes)
...
if __name__ == '__main__':
    ...
    print(set.intersection(set(x['train'].uq_idxs), set(x['ood_train'].uq_idxs)))
    print('Printing total instances in train...')
    print(len(set(x['train'].uq_idxs)) + len(set(x['ood_train'].uq_idxs)))

    print(f'Num Labelled Classes: {len(set(x["train"].targets))}')
    print(f'Num Unabelled Classes: {len(set(x["ood_train"].targets))}')
    print(f'Len labelled set: {len(x["train"])}')
    print(f'Len unlabelled set: {len(x["ood_train"])}')