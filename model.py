...
class DINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bb = True, use_bn=False, norm_last_layer=True, 
        ...
        #self.last_layer.parametrizations.weight[0].g.data.fill_(1)
        if norm_last_layer:
    ...
    def forward(self, x):
        x_proj = self.mlp(x)
        normal_x = nn.functional.normalize(x, dim=-1, p=2)
        # x = x.detach()
        if self.use_bb:
            logits = self.last_layer(normal_x)
        else:
            logits = self.last_layer(x_proj)
        return normal_x, x_proj, logits
...
#     """
#     A clustering model that receives a 2D input feature tensor (batch_size, feature_dim)
#     and outputs the projected hidden features and clustering results (logits mapping to clusters).
#     """
#     def __init__(self, in_dim, num_clusters, hidden_dim=512):
...
#         """
#         Args:
#             x: 2D input feature of shape (batch_size, in_dim)
#         Returns:
#             hidden_features: projected features of shape (batch_size, hidden_dim)
#             logits: clustering assignment logits of shape (batch_size, num_clusters)
#         """
#         hidden_features = self.projector(x)
#         logits = self.cluster_head(hidden_features)
