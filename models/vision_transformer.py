...
class VisionTransformer(nn.Module):
    ...
    def get_specific_layers(self, x, nth_layers=None):
        x = self.prepare_tokens(x)
        # we return the output tokens from the `n` last blocks
        output = []
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            if len(self.blocks) - i in nth_layers:
                output.append(self.norm(x))
        return output
...
def vit_small14_dinov3(patch_size=16, **kwargs):
    model = VisionTransformer(
    ...
    return model
def vit_base14_dinov3(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
    ...
    return model
def vit_huge14_dinov3(patch_size=14, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=1280,
        depth=32,
